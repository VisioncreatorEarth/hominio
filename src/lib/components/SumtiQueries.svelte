<script lang="ts">
	...
	interface SumtiQueryResult extends QueryResult {
		id: string;
		klesi?: string; // ckaji.klesi
		cmene?: string; // ckaji.cmene (maybe)
		datni?: {
			...
			<div class="flex-shrink-0 pb-6">
				...
				<div class="flex items-center justify-between">
					<h1 class="text-2xl font-bold text-gray-800">
						{truncate(selectedSumti.datni?.vasru?.toString(), 28) || truncate(selectedSumti.id, 28)}
					</h1>
				</div>
				<p class="mb-3 text-sm text-gray-500">
					<code class="rounded bg-gray-200 px-1 text-xs">{selectedSumti.id}</code>
				</p>
				<div class="mb-4">
					{#if selectedSumti.klesi}
						<span
							class="inline-block rounded-full bg-blue-100 px-3 py-1 text-sm font-medium text-blue-800"
						>
							ckaji: {selectedSumti.klesi}
						</span>
					{/if}
					{#if selectedSumti.datni?.klesi}
						<span
							class="ml-2 inline-block rounded-full bg-green-100 px-3 py-1 text-sm font-medium text-green-800"
						>
							datni: {selectedSumti.datni.klesi}
						</span>
					{/if}
				</div>
				...
				<div class="mb-6 rounded border border-gray-300 bg-white p-4 shadow-sm">
					<h2 class="mb-3 text-xl font-semibold text-gray-700">Concept Details</h2>
					<div class="rounded bg-gray-50 p-4">
						{#if selectedSumti.datni}
							{#if selectedSumti.datni.klesi === 'concept'}
								<div class="mb-2">
									<p class="text-sm text-gray-600">
										This is a concept sumti. Relationships are listed below.
									</p>
									{#if selectedSumti.datni.vasru}
										<p class="mt-2 text-sm text-gray-800">
											Description: {selectedSumti.datni.vasru}
										</p>
									{/if}
								</div>
							{:else}
							...
							{/if}
						{:else}
							<p class="text-sm text-gray-500">No data content available</p>
						{/if}
					</div>
				</div>
				...
				<div class="rounded border border-gray-300 bg-white p-4 shadow-sm">
					<h2 class="mb-3 text-xl font-semibold text-gray-700">Relationships</h2>
					{#if isLoadingRelationships}
						<p class="text-sm text-gray-500">Loading relationships...</p>
					{:else if relationshipResults === null}
						<p class="text-sm text-red-600">Error loading relationships.</p>
					{:else if !relationshipResults || relationshipResults.length === 0}
						<p class="text-sm text-yellow-700">No relationships found for this concept.</p>
					{:else}
						<ul class="space-y-4">
							{#each relationshipResults as rel (rel.bridi_id)}
								<li class="rounded border border-gray-200 bg-gray-50 p-3">
									<div class="mb-2">
										<span class="font-medium text-gray-700">Relationship (Bridi):</span>
										<code class="ml-2 rounded bg-gray-200 px-1 text-xs text-gray-600"
											>{truncate(rel.bridi_id)}</code
										>
									</div>
									<div class="mb-3">
										<span class="font-medium text-gray-700">Type (Selbri):</span>
										{#if rel.selbri_resolved}
											<span class="ml-2 text-sm text-gray-800">
												{rel.selbri_resolved.content || 'Unnamed Selbri'}
											</span>
											<code class="ml-1 rounded bg-gray-200 px-1 text-xs text-gray-500"
												>{truncate(rel.selbri_resolved.id)}</code
											>
										{:else}
											<code class="ml-2 rounded bg-gray-200 px-1 text-xs text-gray-600"
												>{truncate(rel.selbri_id)} (unresolved)</code
											>
										{/if}
									</div>
									<div class="space-y-2">
										{#each ['x1', 'x2', 'x3', 'x4', 'x5'] as place}
											...
											{#if resolvedNode && resolvedNode.id}
												<div class="flex items-center space-x-2 text-sm">
													<span
														class="w-8 rounded bg-blue-100 px-1 py-0.5 text-center text-xs font-medium text-blue-800"
														>{place.toUpperCase()}</span
													>
													<code
														class="flex-1 overflow-hidden rounded bg-gray-200 px-1 text-xs text-ellipsis whitespace-nowrap text-gray-600"
														title={resolvedNode.id}>{truncate(resolvedNode.id, 10)}</code
													>
													<span class="text-gray-700">({resolvedNode.klesi || 'no klesi'})</span>
													{#if resolvedNode.content}
														<span class="text-gray-500"
															>:: {truncate(JSON.stringify(resolvedNode.content), 20)}</span
														>
													{/if}
												</div>
											{/if}
